...
| [`cubox-research`](skills/cubox-research/SKILL.md) | 研究 | 以用户的 Cubox/CueBox 收藏为事实来源做主题研究：主动扩展关键词、抓取最相关文章详情，并基于导出的 Markdown 做本地分析。需要 Bun 和已配置的 `.env`。 |
...
- 只要问题应该从用户的 Cubox/CueBox 收藏里找答案，优先使用 `cubox-research`；它会有意识地扩展关键词、抓取最相关文章详情，并在用户明确要求前保持只读。
