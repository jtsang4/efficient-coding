...
export function buildOrchestratorPrompt({ run, task, acceptance = "", pluginRoot }) {
...
所有跨 Agent 操作都通过以下命令完成：

\`${command} spawn --role implementer --task-file <path>\`
\`${command} spawn --role verifier --task-file <path>\`
\`${command} send --agent <name> --task-file <path>\`
\`${command} list\`
\`${command} inbox\`
\`${command} inspect --agent <name>\`
\`${command} read --agent <name>\`
\`${command} ack --all\`
\`${command} retire --agent <name> --reason <text>\`
\`${command} complete --summary-file <path>\`

先形成计划和验收标准，再开始派发。派发后结束当前回合，等待用户消息或插件异步事件通知。`;
}
...
\`${command} submit --status <pass|fail|blocked> --result-file <path>\``
    : `将结构化结果写到一个 JSON 或 Markdown 文件，然后运行：
\`${command} submit --status <completed|blocked> --result-file <path>\``;
