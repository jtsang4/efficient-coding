...
| [`agent-native-redesign`](skills/agent-native-redesign/SKILL.md) | Workflow | Rethink a project or process with abundant agent and token budgets: challenge inherited assumptions, design the ideal operation, then examine gains in quality, automation, and scale and validate critical dependencies. |
...
| [`paseo-relay`](skills/paseo-relay/SKILL.md) | Integration | Use the local Paseo CLI to reach other hosts through authorized Relay pairing information, discover workspaces and agents, read sessions, and perform requested operations. Optional SDK reads add identity verification, profiles, and complete raw timelines. |
...
- To rethink the current project or scenario around abundant agent capacity, use `agent-native-redesign`: challenge historical constraints, describe the ideal operation, then assess feasibility and the new bottlenecks.
...
- For cross-host Paseo access, use `paseo-relay`: obtain or reuse authorized pairing information, verify the target host, then use native `paseo` commands. Observation is the default; remote mutations require the user's authorization. See its [optional SDK reads and validation](skills/paseo-relay/references/sdk.md) for capabilities missing from the CLI.
...
| Lint | Frontend / JS / TS | [Biome](https://biomejs.dev/linter/) | Unified formatter and linter for frontend projects with fast performance and sensible defaults; start with the recommended rules, then tighten conventions as needed. | <https://biomejs.dev/linter/> |
