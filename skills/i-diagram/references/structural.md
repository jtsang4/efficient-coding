...
Snippets use theme tokens (`{bg}`, `{text}`, `{text-sub}`, `{line}`) and `FILL`/`STROKE` (assigned palette category in the active theme) — resolve per the Design System in SKILL.md.
...
<g transform="translate(X, Y)">
  <!-- Mask -->
  <rect width="180" height="120" rx="6" fill="{bg}"/>
  <!-- Box -->
  <rect width="180" height="120" rx="6" fill="FILL" stroke="STROKE" stroke-width="1.5"/>
  <!-- Class name compartment -->
  <text x="90" y="24" fill="{text}" font-size="11" font-weight="700" text-anchor="middle">ClassName</text>
  <!-- Divider 1 -->
  <line x1="0" y1="35" x2="180" y2="35" stroke="STROKE" stroke-width="0.5" stroke-opacity="0.5"/>
  <!-- Attributes -->
  <text x="10" y="52" fill="{text-sub}" font-size="8">- id: int</text>
  <text x="10" y="64" fill="{text-sub}" font-size="8">- name: string</text>
  <!-- Divider 2 -->
  <line x1="0" y1="75" x2="180" y2="75" stroke="STROKE" stroke-width="0.5" stroke-opacity="0.5"/>
  <!-- Methods -->
  <text x="10" y="92" fill="{text-sub}" font-size="8">+ getName(): string</text>
  <text x="10" y="104" fill="{text-sub}" font-size="8">+ setName(s: string)</text>
</g>
```
...
<marker id="inherit" markerWidth="12" markerHeight="10" refX="12" refY="5" orient="auto">
  <polygon points="0 0, 12 5, 0 10" fill="{bg}" stroke="{line}" stroke-width="1.5"/>
</marker>

<!-- Composition diamond -->
<marker id="composition" markerWidth="12" markerHeight="8" refX="0" refY="4" orient="auto">
  <polygon points="0 4, 6 0, 12 4, 6 8" fill="{line}"/>
</marker>

<!-- Aggregation diamond -->
<marker id="aggregation" markerWidth="12" markerHeight="8" refX="0" refY="4" orient="auto">
  <polygon points="0 4, 6 0, 12 4, 6 8" fill="{bg}" stroke="{line}" stroke-width="1.5"/>
</marker>
```
...
```svg
<text x="X" y="Y" fill="{text-sub}" font-size="8">1..*</text>
```
...
```svg
<!-- One end (single line) -->
<line x1="X1" y1="Y" x2="X1+15" y2="Y" stroke="{line}" stroke-width="1.5"/>
<!-- Many end (crow's foot) -->
<line x1="X2-15" y1="Y-6" x2="X2" y2="Y" stroke="{line}" stroke-width="1.5"/>
<line x1="X2-15" y1="Y+6" x2="X2" y2="Y" stroke="{line}" stroke-width="1.5"/>
<line x1="X2-15" y1="Y" x2="X2" y2="Y" stroke="{line}" stroke-width="1.5"/>
```
