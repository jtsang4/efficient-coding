...
import { FlareSolverrClient, convertToPlaywrightCookies } from "./flaresolverr.js";
import { configurePageForAntiDetection } from "./anti-detection.js";
...
/**
 * Options for bypassing Cloudflare protection
 */
export interface BypassCloudflareOptions {
  /** FlareSolverr client instance or config */
  flaresolverr?: FlareSolverrClient | { baseUrl?: string; timeout?: number };
  /** Session ID to use for persistence */
  sessionId?: string;
  /** Maximum time to wait for challenge resolution */
  maxTimeout?: number;
  /** Whether to clear existing cookies before injection */
  clearExistingCookies?: boolean;
}

/**
 * Bypass Cloudflare protection for a URL using FlareSolverr
 *
 * This uses FlareSolverr to solve the Cloudflare challenge and then
 * injects the resulting cookies into the Playwright page.
 *
 * @param page Playwright page instance
 * @param url URL to navigate to
 * @param options Bypass options
 * @returns The solve result with cookies and user agent
 */
export async function bypassCloudflare(
  page: Page,
  url: string,
  options: BypassCloudflareOptions = {}
): Promise<{
  cookies: Array<{
    name: string;
    value: string;
  ...
  let flaresolverr: FlareSolverrClient;
  if (options.flaresolverr instanceof FlareSolverrClient) {
    flaresolverr = options.flaresolverr;
  } else {
    const config = options.flaresolverr || {};
    flaresolverr = new FlareSolverrClient({
  ...
  if (options.clearExistingCookies) {
    const context = page.context();
    await context.clearCookies();
  }

  // Solve the URL with FlareSolverr
  console.log(`Solving Cloudflare challenge for: ${url}`);
  const result = await flaresolverr.solveUrl(url, {
    sessionId: options.sessionId,
    maxTimeout: options.maxTimeout,
  });

  console.log(`Challenge solved! Status: ${result.status}, Got ${result.cookies.length} cookies`);

  // Inject cookies into Playwright context
  const context = page.context();
  const playwrightCookies = convertToPlaywrightCookies(result.cookies, new URL(url).hostname);
  ...
  // Navigate to the URL with the cookies
  await page.goto(url);
...
/**
 * Apply stealth mode anti-detection patches to a page
 *
 * This removes automation flags (like navigator.webdriver) and applies
 * patches to evade bot detection systems.
 *
 * @param page Playwright page instance
 */
export async function applyStealthMode(page: Page): Promise<void> {
  await configurePageForAntiDetection(page);
}

/**
 * Re-export anti-detection utilities
 */
export { FlareSolverrClient, convertToPlaywrightCookies } from "./flaresolverr.js";
export {
  launchBrowser,
  applyStealthArgs,
  getRandomUserAgent,
