...
import { applyStealthArgs, getBrowserDriver, applyStealthPatches } from "./anti-detection.js";
...
// Auto-detect browser driver based on availability and options
async function getChromiumDriver(preferPatchright = true) {
  return getBrowserDriver(preferPatchright);
}
...
export async function serve(options: ServeOptions = {}): Promise<DevBrowserServer> {
  ...
  const driverOption = options.driver ?? "playwright";
  const antiDetection = options.antiDetection;

  // Determine if we should try to use patchright
  const preferPatchright = driverOption === "patchright" || driverOption === "auto";
  ...
  // Get browser driver (patchright or playwright)
  console.log("Launching browser with persistent context...");
  const driver = await getChromiumDriver(preferPatchright);
  const { chromium } = driver;
  const usingPatchright = driver !== (await import("playwright"));

  if (usingPatchright) {
    console.log("✓ Using Patchright (stealth-enabled browser)");
  } else {
    console.log("✓ Using standard Playwright");
  }

  // Prepare browser args
  let browserArgs: string[] = [`--remote-debugging-port=${cdpPort}`];

  // Apply anti-detection args if enabled
  if (antiDetection?.stealth) {
    browserArgs = applyStealthArgs(browserArgs, {
      userAgent: antiDetection.userAgent,
      viewport: antiDetection.viewport,
      locale: antiDetection.flaresolverr ? undefined : "en-US",
      timezone: antiDetection.flaresolverr ? undefined : "America/New_York",
    });
  ...
  // If stealth mode enabled, apply patches to all pages
  if (antiDetection?.stealth && !usingPatchright) {
    console.log("Applying runtime stealth patches...");
    const existingPages = context.pages();
    for (const page of existingPages) {
      await applyStealthPatches(page).catch((err) => {
        console.warn("Failed to apply stealth patches to page:", err);
      });
    }
  }
  ...
  // GET / - server info (extended with anti-detection status)
  app.get("/", (_req: Request, res: Response) => {
    const response: ServerInfoResponse & { antiDetection?: { stealth: boolean; usingPatchright: boolean } } = {
      wsEndpoint,
      antiDetection: antiDetection?.stealth
        ? { stealth: true, usingPatchright }
        : undefined,
    };
    res.json(response);
  });
  ...
  app.post("/pages", async (req: Request, res: Response) => {
    ...
    if (!entry) {
      ...
      // Apply stealth patches if using stealth mode but not patchright
      if (antiDetection?.stealth && !usingPatchright) {
        await applyStealthPatches(page).catch((err) => {
          console.warn("Failed to apply stealth patches:", err);
        });
      }
